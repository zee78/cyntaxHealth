$(document).ready(()=>{
    ...
    var table = $('#tblDailyRecord').DataTable({
          responsive: true,
          dom: "Bfrtip",
           buttons: ['copy', 'excel', 'pdf', 'csv' , 'colvis' , 'print'],
          ajax: {
             "url": "/dastarkhwan/daily-dastarkhwan-record/datatable",
             "dataSrc": ""
         },
         columns: [
           { data : 'id'},
          ...
          // },
           ...
           { render : function(data, type, row , full) {
            // console.log(row)
              return `
              <div class="glyph">
                  <a href="/dastarkhwan/daily-dastarkhwan-record/`+row.id+`/edit"> <i class="typcn typcn-edit"></i> </a>
    ...
    $('#deleteData').on('submit' , function(event){
      ...
      console.log($trendId)
         ...
         $.ajax({
          url: '/dastarkhwan/daily-dastarkhwan-record/'+$trendId,
          type: 'DELETE',
          data: data,
          processData: false,
