$(document).ready(()=>{
    ...
    var table = $('#tblProductData').DataTable({
          ...
          ajax: {
             "url": "/dhaaga-clothings/women-product-registration/datatable",
             "dataSrc": ""
         },
         columns: [
           { data : 'id'},
          ...
          // },
           ...
           { render : function(data, type, row , full) {
            // console.log(row)
              return `
              <div class="glyph">
                  <a href="/dhaaga-clothings/women-product-registration/`+row.id+`/edit"> <i class="typcn typcn-edit"></i> </a>
    ...
    $('#deleteData').on('submit' , function(event){
      ...
      $trendId = $("#productId").val();
      console.log($trendId)

         $.ajax({
          url: '/dhaaga-clothings/women-product-registration/'+$trendId,
          type: 'DELETE',
          data: data,
          processData: false,
